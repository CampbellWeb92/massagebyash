(() => {
  ...
  const updateProgress = () => bookingProgressItems.forEach(item=>{ const step=Number(item.dataset.progressStep); const reached=step<=maxReachedBookingStep; item.dataset.state=step===currentBookingStep?"current":reached?"complete":"future"; item.setAttribute("aria-disabled",String(!reached)); item.tabIndex=reached?0:-1; item.setAttribute("role","button"); });
  const goToBookingStep = (step,scroll=true) => { currentBookingStep=Math.min(4,Math.max(1,Number(step)||1)); maxReachedBookingStep=Math.max(maxReachedBookingStep,currentBookingStep); bookingStepPanels.forEach(panel=>panel.hidden=Number(panel.dataset.bookingStep)!==currentBookingStep); updateProgress(); if(scroll) bookingForm?.scrollIntoView({behavior:"smooth",block:"start"}); };
  ...
  bookingProgressItems.forEach(item=>{
    const openReachedStep=()=>{const step=Number(item.dataset.progressStep);if(step<=maxReachedBookingStep)goToBookingStep(step);};
    item.addEventListener("click",openReachedStep);
    item.addEventListener("keydown",(event)=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();openReachedStep();}});
  });
